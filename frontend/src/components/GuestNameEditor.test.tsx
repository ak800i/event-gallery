...
describe('GuestNameEditor', () => {
  it('shows the edit form immediately when no name is set', () => {
    render(<GuestNameEditor guestName="" onSave={vi.fn()} maxLength={60} branding={DEFAULT_BRANDING} />)
    expect(screen.getByLabelText(/your name/i)).toBeInTheDocument()
  })

  it('shows a display view with a change link when a name is already set', () => {
    render(<GuestNameEditor guestName="Alex" onSave={vi.fn()} maxLength={60} branding={DEFAULT_BRANDING} />)
  ...
  it('calls onSave with the trimmed name on submit', async () => {
    ...
    render(<GuestNameEditor guestName="" onSave={onSave} maxLength={60} branding={DEFAULT_BRANDING} />)
