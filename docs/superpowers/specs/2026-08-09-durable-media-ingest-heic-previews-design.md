...
existed — is a client-facing 4xx carrying no `Retry-After`, and never 409, 423,
or 429. `@uppy/tus` installs its own `onShouldRetry` and never falls through to
tus-js-client's; that predicate retries those three, so they cannot express a
terminal refusal. Without that terminal answer a permanently unpromotable
